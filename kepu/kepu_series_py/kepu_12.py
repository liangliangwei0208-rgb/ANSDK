...
输出：
kepu/series/kepu_limit_signal.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_limit_signal.png"
...
def _draw_core_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "一句话：限购说明申购被限制了，但不等于基金公司在暗示“后面要涨”。",
        font_size=66,
        min_font_size=48,
        bold=True,
        fill=art.INK,
...
def _draw_reason_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "重点：限购更多是基金运作和申购管理规则，不是基金未来涨跌的保证。",
        font_size=60,
        min_font_size=42,
        bold=True,
        fill=art.INK,
...
def _draw_judgement_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    bullets = [
        "先看限购类型：是单日限额、大额限购、暂停申购，还是影响定投？",
        "再看公告原因：是规模控制、额度限制、假期安排，还是产品运作需要？",
        "不要因为“买不到”就追：热门不等于便宜，限购不等于后面一定涨。",
        "已经持有的人，也不要只因为限购就加仓；仍要看风险、估值和仓位。",
    ]
    ...
    _draw_text_in_box(
        ...
        "简单记：限购不是上涨暗号，只是申购规则变了；先看公告，再看风险。",
        font_size=62,
        min_font_size=44,
        bold=True,
        fill=art.RED,
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；具体申购规则以基金公告、合同和销售平台展示为准。",
        font_size=64,
        min_font_size=44,
        bold=True,
        fill=art.RED,
...
def run(today=None) -> bool:
    ...
    print(f"基金限购误区科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：基金限购是不是说明要涨")
    parser.add_argument(
        "--today",
        default=None,
