...
输出：
kepu/series/kepu_us_stock_not_up.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_us_stock_not_up.png"
...
def _draw_first_check_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "一句话：美股涨，不代表你这只海外基金当天一定涨；先看口径有没有对上。",
        font_size=66,
        min_font_size=48,
        bold=True,
        fill=art.INK,
...
def _draw_reasons_section(draw: ImageDraw.ImageDraw) -> None:
    left, content_top, right, section_bottom = _draw_section_shell(
        draw,
        1230,
        850,
    ...
    _draw_text_in_box(
        ...
        "重点：美股涨只是一个背景，基金最终涨跌要看净值日期、跟踪标的、汇率和真实持仓。",
        font_size=58,
        min_font_size=42,
        bold=True,
        fill=art.INK,
...
def _draw_checklist_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    bullets = [
        "第一，看基金名称和详情页：它跟踪的是纳指、标普、半导体，还是其他海外资产",
        "第二，看净值日期：当前显示的净值，是否已经反映昨晚美股收盘？",
        "第三，看人民币汇率：美元资产上涨，但汇率变化可能抵消部分人民币收益。",
        "第四，看估值口径：平台估值只是参考，最终仍以基金公司公告净值为准。",
    ]
    ...
    _draw_text_in_box(
        ...
        "简单记：先对标的，再对日期，再看汇率；不要只用“美股涨了”判断基金。",
        font_size=60,
        min_font_size=42,
        bold=True,
        fill=art.RED,
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；具体净值披露和涨跌幅以基金公告、合同和销售平台展示为准。",
        font_size=62,
        min_font_size=42,
        bold=True,
        fill=art.RED,
...
def run(today=None) -> bool:
    ...
    print(f"美股涨海外基金没涨科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：美股涨了为什么海外基金没涨")
    parser.add_argument(
        "--today",
        default=None,
