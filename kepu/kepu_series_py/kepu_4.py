...
输出：
kepu/series/kepu_update_time.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_update_time.png"
...
def _draw_process_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    steps = [
        ("市场收盘", "当天价格确定", art.BLUE),
        ("基金核算", "计算资产净值", art.GOLD),
        ("托管复核", "核对数据口径", art.GREEN),
    ...
    for idx, (title, sub, color) in enumerate(steps):
        ...
        _draw_text_in_box(
            draw,
            (x + 25, box_y + 34, x + box_w - 25, box_y + box_h - 24),
            f"{title}\n{sub}",
            font_size=54,
            min_font_size=38,
            bold=True,
        ...
        if idx < len(steps) - 1:
            art.draw_arrow(draw, x + box_w + 24, box_y + box_h // 2, x + box_w + gap - 22)

    note_box = (left + 120, box_y + box_h + 58, right - 120, section_bottom - 55)
    art.rounded(draw, note_box, 30, "#ffffff", art.LINE, 2)

    bullets = [
        "普通基金通常在交易日晚上或之后更新收益，不同平台显示时间可能不同。",
        "15:00 前后主要影响申购、赎回按哪天净值确认，不是收益马上更新的时间。",
        "周末和节假日没有新的交易日净值时，账户收益可能不会像工作日一样更新。",
    ]
...
def _draw_special_case_section(draw: ImageDraw.ImageDraw) -> None:
    left, content_top, right, section_bottom = _draw_section_shell(
        draw,
        2110,
        790,
    ...
    bullets = [
        "QDII 和海外基金：还要看海外市场收盘、汇率换算和公告节奏。",
        "节假日：国内休市、海外开盘时，可能出现节后补更新。",
        "估值偏差：平台估值参考旧持仓，最终净值可能和盘中估值不同。",
        "平台同步时间：基金公司披露后，不同销售平台显示时间也可能有差异。",
    ]
    ...
    _draw_text_in_box(
        ...
        "简单记：白天看估值，晚上等净值；最终以基金公告和平台展示为准。",
        font_size=62,
        min_font_size=44,
        bold=True,
        fill=art.RED,
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；最终以基金公告和销售平台展示为准。",
        font_size=72,
        min_font_size=50,
        bold=True,
        fill=art.RED,
...
def build_image(today: date) -> Image.Image:
    ...
    _draw_special_case_section(draw)
    _draw_footer(draw)
...
def run(today=None) -> bool:
    ...
    print(f"基金收益更新时间科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：为什么基金收益晚上才更新")
    parser.add_argument(
        "--today",
        default=None,
