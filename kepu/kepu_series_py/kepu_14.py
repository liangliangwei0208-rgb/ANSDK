...
输出：
kepu/series/kepu_dingtou_limit.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_dingtou_limit.png"
...
def _draw_title(draw: ImageDraw.ImageDraw, today: date) -> None:
    ...
    _draw_center_text(
        ...
        art.WIDTH // 2,
        330,
        "先看公告是否“含定投”，再看金额有没有超限",
        font_subtitle,
        art.MUTED,
    )


def _draw_first_check_section(draw: ImageDraw.ImageDraw) -> None:
...
def _draw_possible_results_section(draw: ImageDraw.ImageDraw) -> None:
    left, content_top, right, section_bottom = _draw_section_shell(
        draw,
        1230,
        850,
    ...
    _draw_text_in_box(
        ...
        "重点：定投也是一种申购申请；限购如果包含定投，就不能只看原来的定投计划。",
        font_size=58,
        min_font_size=42,
        bold=True,
        fill=art.INK,
...
def _draw_action_section(draw: ImageDraw.ImageDraw) -> None:
    left, content_top, right, section_bottom = _draw_section_shell(
        draw,
        2130,
        770,
    ...
    bullets = [
        "先看平台提示：这期定投是成功、失败，还是待确认，不要只看银行卡扣款。",
        "再看定投金额：如果超过限额，可以考虑调低每期金额，而不是硬追。",
        "检查扣款规则：失败后是否自动重试、是否顺延、是否需要手动恢复。",
        "不要绕限购追买：拆多笔、换账户、换平台，可能偏离原来的投资计划。",
    ]
    ...
    _draw_text_in_box(
        ...
        "简单记：先看是否含定投，再看金额是否超限，最后看平台是否扣款成功。",
        font_size=62,
        min_font_size=44,
        bold=True,
        fill=art.RED,
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；具体申购、定投和赎回规则以基金公告、合同和销售平台展示为准。",
        font_size=64,
        min_font_size=44,
        bold=True,
        fill=art.RED,
...
def build_image(today: date) -> Image.Image:
    ...
    _draw_first_check_section(draw)
    _draw_possible_results_section(draw)
    _draw_action_section(draw)
    _draw_footer(draw)
...
def run(today=None) -> bool:
    ...
    print(f"基金定投限购科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：定投遇到限购怎么办")
    parser.add_argument(
        "--today",
        default=None,
