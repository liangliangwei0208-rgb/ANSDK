...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_low_nav.png"
...
def _draw_core_misunderstanding_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "一句话：基金净值低，只说明每一份价格低，不说明它更安全、更便宜或更容易涨。",
        font_size=64,
        min_font_size=46,
        bold=True,
        fill=art.INK,
...
def _draw_same_money_section(draw: ImageDraw.ImageDraw) -> None:
    left, content_top, right, section_bottom = _draw_section_shell(
        draw,
        1230,
        850,
    ...
    for idx, (title, color, body) in enumerate(cards):
        ...
        _draw_text_in_box(
            draw,
            (x + 35, card_y + 48, x + card_w - 35, card_y + 128),
            title,
            font_size=60,
            min_font_size=44,
            bold=True,
        ...
        _draw_text_in_box(
            draw,
            (x + 35, card_y + 155, x + card_w - 35, card_y + card_h - 35),
            body,
            font_size=54,
            min_font_size=40,
            bold=True,
    ...
    _draw_text_in_box(
        ...
        "重点：同样投入下，收益主要看涨跌幅，不是看净值低不低、份额多不多。",
        font_size=60,
        min_font_size=42,
        bold=True,
        fill=art.INK,
...
def _draw_real_selection_section(draw: ImageDraw.ImageDraw) -> None:
    left, content_top, right, section_bottom = _draw_section_shell(
        draw,
        2130,
        770,
    ...
    bullets = [
        "看投向：它主要买股票、债券、海外资产，还是货币工具？",
        "看风险：波动和回撤自己能不能接受，而不是只看净值高低。",
        "看业绩口径：长期表现、同类排名、最大回撤，比单个净值数字更重要。",
        "看费用和规则：A/C份额、申赎费、限购、QDII时间差等都要看清楚。",
    ]
    ...
    _draw_text_in_box(
        ...
        "简单记：基金没有单纯的贵和便宜，关键看它买什么、怎么涨跌、适不适合你。",
        font_size=60,
        min_font_size=42,
        bold=True,
        fill=art.RED,
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；基金过往表现不代表未来表现。",
        font_size=72,
        min_font_size=50,
        bold=True,
        fill=art.RED,
...
def build_image(today: date) -> Image.Image:
    ...
    _draw_core_misunderstanding_section(draw)
    _draw_same_money_section(draw)
    _draw_real_selection_section(draw)
    _draw_footer(draw)
...
def run(today=None) -> bool:
    ...
    print(f"基金低净值误区科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：基金净值低是不是更便宜")
    parser.add_argument(
        "--today",
        default=None,
