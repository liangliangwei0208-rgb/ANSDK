...
输出：
kepu/series/kepu_fund_dingtou_profit.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_fund_dingtou_profit.png"
...
def _draw_core_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "一句话：定投可能赚钱，但不保证赚钱；它解决的是“怎么分批买”，不是“买什么都赚”。",
        font_size=64,
        min_font_size=46,
        bold=True,
        fill=art.INK,
...
def _draw_choice_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "重点：定投的优势是降低择时难度，不是消灭风险；基金本身不好，定投也救不了。",
        font_size=58,
        min_font_size=42,
        bold=True,
        fill=art.INK,
...
def _draw_mistakes_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    bullets = [
        "误区一：定投一定赚钱。实际仍然可能亏损，基金份额持有人要承担市场风险。",
        "误区二：跌了就马上停止。下跌时反而可能买到更多份额，但前提是基金逻辑没坏。",
        "误区三：只会买，不会卖。定投也需要止盈计划，否则盈利可能坐过山车。",
        "误区四：任何基金都适合定投。长期向下、质量很差的基金，不会因为定投就变好。",
    ]
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；定投不保证收益，具体产品风险请以基金公告和销售平台展示为准。",
        font_size=66,
        min_font_size=46,
        bold=True,
        fill=art.RED,
...
def run(today=None) -> bool:
    ...
    print(f"基金定投赚钱逻辑科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：基金定投真的能赚钱吗")
    parser.add_argument(
        "--today",
        default=None,
