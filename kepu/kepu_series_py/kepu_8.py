...
输出：
kepu/series/kepu_friday_buy.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_friday_buy.png"
...
def _draw_title(draw: ImageDraw.ImageDraw, today: date) -> None:
    ...
    _draw_center_text(
        ...
        art.WIDTH // 2,
        330,
        "关键看 15:00、基金类型、周末和节假日顺延",
        font_subtitle,
        art.MUTED,
    )


def _draw_three_questions_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "一句话：周五买基金不一定有问题，但最容易被周末和 15:00 规则绕晕。",
        font_size=66,
        min_font_size=48,
        bold=True,
        fill=art.INK,
...
def _draw_fund_type_section(draw: ImageDraw.ImageDraw) -> None:
    left, content_top, right, section_bottom = _draw_section_shell(
        draw,
        1230,
        850,
    ...
    _draw_text_in_box(
        ...
        "重点不是“周五一定不能买”，而是要看确认日、收益起算日和后面是否休市。",
        font_size=60,
        min_font_size=42,
        bold=True,
        fill=art.INK,
...
def _draw_misunderstanding_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    bullets = [
        "周五 15:00 后买，通常不是按周五净值，而是看下一个交易日。",
        "货币基金周五买，通常要等下周一确认并开始算收益，容易错过周末。",
        "普通基金周末通常没有新的交易日净值，不要期待周六周日每天更新。",
        "周五买不代表一定亏或一定赚，它只是时间规则更容易被误解。",
    ]
    ...
    _draw_text_in_box(
        ...
        "简单记：周五买基金，先看 15:00，再看基金类型，最后看周末和节假日。",
        font_size=62,
        min_font_size=44,
        bold=True,
        fill=art.RED,
...
def build_image(today: date) -> Image.Image:
    ...
    _draw_three_questions_section(draw)
    _draw_fund_type_section(draw)
...
def run(today=None) -> bool:
    ...
    print(f"周五买基金注意事项科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：周五买基金为什么要注意")
    parser.add_argument(
        "--today",
        default=None,
