...
输出：
kepu/series/kepu_1500_rule.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_1500_rule.png"
...
def _draw_core_rule_section(draw: ImageDraw.ImageDraw) -> None:
...
def _draw_buy_sell_section(draw: ImageDraw.ImageDraw) -> None:
    left, content_top, right, section_bottom = _draw_section_shell(
        draw,
        1240,
        810,
    ...
    for idx, (title, color, bullets) in enumerate(cards):
        ...
        _draw_bullets_in_box(
            draw,
            (x + 82, card_y + 178, x + card_w - 60, card_y + card_h - 48),
            bullets,
            color,
            font_size=58,
            min_font_size=44,
    ...
    _draw_text_in_box(
        ...
        "15:00 是申请日分界线，不是收益更新时间；收益通常要等晚上净值更新后才变化。",
        font_size=60,
        min_font_size=42,
        bold=True,
        fill=art.INK,
...
def _draw_misunderstanding_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    bullets = [
        "不是 15:00 前买就一定赚钱，也不是 15:00 后买就一定吃亏。",
        "15:00 只决定申请按哪天净值确认，不预测后面涨跌。",
        "场内 ETF、货币基金、QDII 和特殊基金，规则可能和普通场外基金不同。",
        "定投也要看实际扣款和提交时间，最终以销售平台展示为准。",
    ]
    ...
    _draw_text_in_box(
        ...
        "简单记：15:00 前看当天，15:00 后看下一交易日；最终以平台和公告为准。",
        font_size=62,
        min_font_size=44,
        bold=True,
        fill=art.RED,
...
def build_image(today: date) -> Image.Image:
    ...
    _draw_core_rule_section(draw)
    _draw_buy_sell_section(draw)
...
def run(today=None) -> bool:
    ...
    print(f"基金15点规则科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：15:00 前后买基金有什么区别")
    parser.add_argument(
        "--today",
        default=None,
