...
输出：
kepu/series/kepu_average_down.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_average_down.png"
...
def _draw_three_questions_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "一句话：补仓不是回本按钮，它只是让你的仓位变大、平均成本变化。",
        font_size=66,
        min_font_size=48,
        bold=True,
        fill=art.INK,
...
def _draw_scenarios_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "重点不是“跌了就补”，而是补仓前先确定：资金、仓位、风险和买入理由都能匹配。",
        font_size=58,
        min_font_size=42,
        bold=True,
        fill=art.INK,
...
def _draw_misunderstanding_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    bullets = [
        "把补仓当成回本按钮：补仓只能改变成本，不能保证基金上涨。",
        "越跌越补没有上限：仓位越补越重，继续下跌时亏损金额可能更大。",
        "用短期要用的钱补仓：市场不一定按你的用钱时间反弹。",
        "只因为亏损不甘心就补：这不是计划，是情绪操作。",
    ]
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；请结合自身风险承受能力判断。",
        font_size=72,
        min_font_size=50,
        bold=True,
        fill=art.RED,
...
def run(today=None) -> bool:
    ...
    print(f"基金补仓科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：基金跌了要不要补仓")
    parser.add_argument(
        "--today",
        default=None,
