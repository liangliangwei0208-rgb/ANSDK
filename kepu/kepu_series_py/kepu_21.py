...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_ac_share.png"
...
def _draw_core_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "一句话：A 类常见是买入时收申购费；C 类常见是不收申购费，但每天计提销售服务费。",
        font_size=64,
        min_font_size=46,
        bold=True,
        fill=art.INK,
...
def _draw_choice_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "重点：没有绝对答案，要拿实际费率表算；同一基金不同平台费率也可能不同。",
        font_size=58,
        min_font_size=42,
        bold=True,
        fill=art.INK,
...
def _draw_mistakes_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    bullets = [
        "误区一：C 类 0 申购费就是免费。实际通常还有销售服务费，每天从净值里计提。",
        "误区二：A 类有申购费就一定更贵。长期持有时，C 类销售服务费可能持续累积。",
        "误区三：A/C 是两只完全不同的基金。多数时候只是同一基金的不同收费份额。",
        "误区四：只看买入费，不看赎回费。卖出时间不同，赎回费差异也可能很大。",
    ]
    ...
    _draw_text_in_box(
        ...
        "简单记：短期常看 C，长期常看 A；最终以费率表和持有时间为准。",
        font_size=62,
        min_font_size=44,
        bold=True,
        fill=art.RED,
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；具体费率以基金公告、合同和销售平台展示为准。",
        font_size=66,
        min_font_size=46,
        bold=True,
        fill=art.RED,
...
def run(today=None) -> bool:
    ...
    print(f"基金A类C类区别科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：基金 A 类和 C 类有什么区别")
    parser.add_argument(
        "--today",
        default=None,
