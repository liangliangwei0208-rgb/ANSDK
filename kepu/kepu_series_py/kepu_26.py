...
输出：
kepu/series/kepu_max_drawdown.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_max_drawdown.png"
...
def _draw_core_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "一句话：最大回撤，就是某段时间里，从阶段高点跌到后面低点，最多亏了多少比例。",
        font_size=64,
        min_font_size=46,
        bold=True,
        fill=art.INK,
...
def _draw_choice_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "重点：最大回撤是历史指标，只说明过去最深跌幅，不代表未来最多只会跌这么多。",
        font_size=58,
        min_font_size=42,
        bold=True,
        fill=art.INK,
...
def _draw_mistakes_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    bullets = [
        "误区一：最大回撤就是最终亏损。实际它看的是历史过程中从高点到低点的最大跌幅。",
        "误区二：回撤小就一定收益低。低回撤只说明波动压力较小，不等于收益一定差。",
        "误区三：回撤大就一定不能买。要结合收益、波动、投资方向和自己的承受能力看。",
        "误区四：历史最大回撤能预测未来。它只能参考过去，未来极端行情可能更差。",
    ]
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；最大回撤为历史风险参考，具体产品风险请以基金公告和销售平台展示为准。",
        font_size=66,
        min_font_size=46,
        bold=True,
        fill=art.RED,
...
def run(today=None) -> bool:
    ...
    print(f"最大回撤科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：最大回撤是什么意思")
    parser.add_argument(
        "--today",
        default=None,
