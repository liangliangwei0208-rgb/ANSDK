...
输出：
kepu/series/kepu_dingtou_take_profit.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_dingtou_take_profit.png"
...
def _draw_core_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "一句话：定投可以长期买，但盈利后也要管理；止盈不是清仓逃跑，而是锁住部分成果。",
        font_size=64,
        min_font_size=46,
        bold=True,
        fill=art.INK,
...
def _draw_mistakes_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    bullets = [
        "误区一：定投就是一直买不卖。实际长期持有也需要阶段性检查和止盈纪律。",
        "误区二：一赚钱就全部卖掉。过早清仓可能错过后续行情，可以考虑分批止盈。",
        "误区三：必须卖在最高点。没人能稳定卖在最高点，止盈看的是纪律，不是神预测。",
        "误区四：止盈后就结束了。止盈后的钱也要规划，避免又在高位一次性买回去。",
    ]
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；定投不保证收益，是否止盈应结合自身风险承受能力和资金安排。",
        font_size=66,
        min_font_size=46,
        bold=True,
        fill=art.RED,
...
def run(today=None) -> bool:
    ...
    print(f"定投止盈科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：定投要不要止盈")
    parser.add_argument(
        "--today",
        default=None,
