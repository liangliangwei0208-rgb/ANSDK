...
输出：
kepu/series/kepu_notice_key_lines.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_notice_key_lines.png"
...
def _draw_title(draw: ImageDraw.ImageDraw, today: date) -> None:
    ...
    _draw_center_text(
        ...
        art.WIDTH // 2,
        330,
        "别全文硬读，先抓标题、日期、业务、限额和恢复安排",
        font_subtitle,
        art.MUTED,
    )


def _draw_first_section(draw: ImageDraw.ImageDraw) -> None:
...
def _draw_key_lines_section(draw: ImageDraw.ImageDraw) -> None:
    left, content_top, right, section_bottom = _draw_section_shell(
        draw,
        1230,
        850,
...
def _draw_mistakes_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    bullets = [
        "只看标题不看日期：公告可能明天生效，也可能已经结束。",
        "只看暂停申购不看赎回：不能买，不一定代表不能卖。",
        "只看限额不看范围：定投、转换转入、A/C份额可能分别限制。",
        "只看收益不看风险：分红、净值、费率变化，都不等于稳赚。",
    ]
    ...
    _draw_text_in_box(
        ...
        "简单记：标题看影响，日期看时间，业务看能不能买卖，限额看能买多少。",
        font_size=60,
        min_font_size=42,
        bold=True,
        fill=art.RED,
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；具体规则以基金公告、合同和销售平台展示为准。",
        font_size=66,
        min_font_size=46,
        bold=True,
        fill=art.RED,
...
def build_image(today: date) -> Image.Image:
    ...
    _draw_first_section(draw)
    _draw_key_lines_section(draw)
...
def run(today=None) -> bool:
    ...
    print(f"基金公告重点行科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：基金公告应该重点看哪几行")
    parser.add_argument(
        "--today",
        default=None,
