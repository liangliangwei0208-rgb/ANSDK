...
输出：
kepu/series/kepu_fund_convert_vs_sell_buy.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_fund_convert_vs_sell_buy.png"
...
def _draw_core_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "一句话：基金转换像“一步换基金”；先卖再买是“先赎回到账，再重新申购”。",
        font_size=64,
        min_font_size=46,
        bold=True,
        fill=art.INK,
...
def _draw_choice_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "重点：转换不等于免费，也不等于一定更划算；要看页面试算和费率规则。",
        font_size=58,
        min_font_size=42,
        bold=True,
        fill=art.INK,
...
def _draw_mistakes_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    bullets = [
        "误区一：基金转换一定不要钱。实际可能仍有赎回费，也可能有申购费补差。",
        "误区二：所有基金都能互相转换。通常要在平台支持范围内，且转出可赎回、转入可申购。",
        "误区三：转换后还沿用老基金持有天数。转入份额的持有期通常会重新计算。",
        "误区四：转换一定比先卖再买更好。若可选范围、费率、到账时间不同，结果也可能不同。",
    ]
    ...
    _draw_text_in_box(
        ...
        "简单记：想省步骤看转换，想自由选择看先卖再买；最终看费用和规则。",
        font_size=62,
        min_font_size=44,
        bold=True,
        fill=art.RED,
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；具体转换规则、确认时间和费用以基金公告及交易页面为准。",
        font_size=66,
        min_font_size=46,
        bold=True,
        fill=art.RED,
...
def run(today=None) -> bool:
    ...
    print(f"基金转换和先卖再买区别科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：基金转换和先卖再买有什么区别")
    parser.add_argument(
        "--today",
        default=None,
