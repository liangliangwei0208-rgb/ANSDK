...
输出：
kepu/series/kepu_holiday_overseas_open.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_holiday_overseas_open.png"
...
def _draw_title(draw: ImageDraw.ImageDraw, today: date) -> None:
    ...
    _draw_center_text(
        ...
        art.WIDTH // 2,
        330,
        "海外涨跌不会消失，只是通常等节后净值再体现",
        font_subtitle,
        art.MUTED,
    )


def _draw_holiday_logic_section(draw: ImageDraw.ImageDraw) -> None:
...
def _draw_update_section(draw: ImageDraw.ImageDraw) -> None:
    left, content_top, right, section_bottom = _draw_section_shell(
        draw,
        1230,
        850,
    ...
    _draw_text_in_box(
        ...
        "重点：节后涨跌可能包含假期内海外市场、汇率和节后交易日的综合影响。",
        font_size=58,
        min_font_size=42,
        bold=True,
        fill=art.INK,
...
def _draw_misunderstanding_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    bullets = [
        "误区一：国内放假，海外涨跌就和基金没关系。实际底层资产仍会影响基金。",
        "误区二：假期账户没变化，说明没有收益变化。很多时候只是没有披露新净值。",
        "误区三：节后大涨大跌就是一天造成的。它可能包含假期多天累计影响。",
        "误区四：节后第一晚没显示，就是没算进去。QDII 净值披露本来可能更慢。",
    ]
    ...
    _draw_text_in_box(
        ...
        "简单记：国内假期不更新，不代表海外收益没变化；节后看净值日期和公告。",
        font_size=60,
        min_font_size=42,
        bold=True,
        fill=art.RED,
...
def build_image(today: date) -> Image.Image:
    ...
    _draw_holiday_logic_section(draw)
    _draw_update_section(draw)
...
def run(today=None) -> bool:
    ...
    print(f"国内假期海外开盘科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：国内放假海外开盘基金收益怎么算")
    parser.add_argument(
        "--today",
        default=None,
