...
输出：
kepu/series/kepu_fund_liquidation.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_fund_liquidation.png"
...
def _draw_core_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "一句话：基金清盘就是基金不再继续运作，进入清算，把剩余资产按份额分给持有人。",
        font_size=64,
        min_font_size=46,
        bold=True,
        fill=art.INK,
...
def _draw_choice_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "重点：清盘不是单纯“亏没了”，更常见是基金规模、人数或合同条件触发终止。",
        font_size=58,
        min_font_size=42,
        bold=True,
        fill=art.INK,
...
def _draw_mistakes_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    bullets = [
        "误区一：清盘就是钱没了。实际通常是资产清算后，剩余部分按份额分回投资者。",
        "误区二：清盘一定是基金经理跑路。公募基金财产独立托管，清盘是合同终止后的清算流程。",
        "误区三：清盘就不会亏。若清盘时净值低于买入成本，拿回的钱仍可能低于本金。",
        "误区四：看到清盘公告就不管了。要关注赎回、转换、清算到账时间和基金公告安排。",
    ]
    ...
    _draw_text_in_box(
        ...
        "简单记：清盘不是钱消失，而是基金结束；能拿回多少，看清算时剩余资产。",
        font_size=62,
        min_font_size=44,
        bold=True,
        fill=art.RED,
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；基金清盘、赎回、转换和到账安排请以基金公告及销售平台展示为准。",
        font_size=66,
        min_font_size=46,
        bold=True,
        fill=art.RED,
...
def run(today=None) -> bool:
    ...
    print(f"基金清盘科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：基金清盘是什么意思")
    parser.add_argument(
        "--today",
        default=None,
