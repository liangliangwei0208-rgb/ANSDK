...
输出：
kepu/series/kepu_suspend_purchase_redeem.png
"""
...
OUTPUT_FILE = KEPU_DIR / "series" / "kepu_suspend_purchase_redeem.png"
...
def _draw_core_section(draw: ImageDraw.ImageDraw) -> None:
    ...
    _draw_text_in_box(
        ...
        "一句话：暂停申购主要是限制买入和追加，不等于这只基金一定不能卖出。",
        font_size=66,
        min_font_size=48,
        bold=True,
        fill=art.INK,
...
def _draw_cannot_sell_section(draw: ImageDraw.ImageDraw) -> None:
    left, content_top, right, section_bottom = _draw_section_shell(
        draw,
        1230,
        850,
...
def _draw_judgement_section(draw: ImageDraw.ImageDraw) -> None:
    left, content_top, right, section_bottom = _draw_section_shell(
        draw,
        2130,
        770,
    ...
    bullets = [
        "看公告标题：是“暂停申购”，还是“暂停申购、赎回”。",
        "看平台按钮：申购按钮灰了，不代表赎回按钮也一定灰。",
        "看业务范围：定投、转换转入、转换转出，可能分别有不同限制。",
        "看基金类型：QDII、定开基金、持有期基金和场内ETF，规则可能不同。",
    ]
...
def _draw_footer(draw: ImageDraw.ImageDraw, top: int = 3000) -> None:
    ...
    _draw_text_in_box(
        ...
        "仅供个人学习记录，不构成任何投资建议；具体申购赎回规则以基金公告、合同和销售平台展示为准。",
        font_size=64,
        min_font_size=44,
        bold=True,
        fill=art.RED,
...
def build_image(today: date) -> Image.Image:
    ...
    _draw_cannot_sell_section(draw)
    _draw_judgement_section(draw)
    _draw_footer(draw)
...
def run(today=None) -> bool:
    ...
    print(f"基金暂停申购赎回科普图已生成: {OUTPUT_FILE.resolve()}")
    return True


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成基金科普图：暂停申购还能不能卖出")
    parser.add_argument(
        "--today",
        default=None,
