...
#默认中文名称映射；可以在 rsi_analyze_index(display_name=...) 中手动覆盖
...
def _fetch_cn_etf(symbol: str, days: int) -> pd.DataFrame:
    ...
    #一、新浪 ETF 历史行情：返回 date/open/high/low/close/volume
    try:
        if s.startswith("5"):
            sina_symbol = "sh" + s
        elif s.startswith("1"):
    ...
    #二、东方财富 ETF 历史行情：字段完整，包含成交量、成交额
    try:
...
#使用 Yahoo Finance 获取指数历史数据，作为非东方财富备用源
def _fetch_yahoo_index(symbol: str, days: int) -> pd.DataFrame:
    ...
    try:
        ...
        yf = None
    # 一、优先使用 yfinance，避免 Yahoo chart API 直接 requests 被 403 拦截
    if yf is not None:
        try:
            df = yf.download(
                yahoo_symbol,
    ...
    # 二、备用：requests 直连 Yahoo chart API
...
def get_index_akshare(
    ...
    def try_fetch_once() -> pd.DataFrame:
        ...
        #二、中国场内 ETF：例如 512890、510300、159915
        ...
        #三、Yahoo Finance：优先用于 H30269 这类 CSI 指数
        ...
        #五、东方财富接口：默认不用，只有 allow_eastmoney=True 才启用
        if allow_eastmoney:
            try:
...
def resample_ohlcv(
    ...
    #保留周期内最后一个实际交易日，用于后续把周线/月线 RSI 信号标注到日线图上
...
def print_latest_summary(
    ...
    #print(df[show_cols].tail(5))


#根据 RSI 阈值连续变色绘制价格或 RSI 曲线
def _plot_segmented_by_rsi(
    ...
    rsi_col: str,#RSI列
    date_col: str, #日期列
    rsi_high: float,#RSI高阈值
    rsi_low: float,#RSI低阈值
    ylabel: str,#y轴标题
    title: str,#图标题
    linewidth: float = 1.8,
    ...
    """
    cols = list(dict.fromkeys([date_col, y_col, rsi_col]))#按照列名提取数据
    plot_df = df[cols].copy()
    #确保日期列为 datetime，数值列为 float，且丢弃任何缺失值
    plot_df[date_col] = pd.to_datetime(plot_df[date_col], errors="coerce")
    #print(f"转换 { plot_df[date_col].name } 列为数值，非数值将被设置为 NaN")
    plot_df[y_col] = pd.to_numeric(plot_df[y_col], errors="coerce")
    #print(f"转换 { plot_df[y_col].name } 列为数值，非数值将被设置为 NaN")
    #如果 RSI 列与 y_col 不同，也尝试转换为数值
    if rsi_col != y_col:
        plot_df[rsi_col] = pd.to_numeric(plot_df[rsi_col], errors="coerce")
    #丢弃任何在日期、y_col 或 rsi_col 中缺失的行，并重置索引
    plot_df = plot_df.dropna(subset=[date_col, y_col, rsi_col]).reset_index(drop=True)
    #如果数据少于 2 行，则不绘制任何内容并返回
    if len(plot_df) < 2:
        return

    x = mdates.date2num(plot_df[date_col])#将日期转换为 Matplotlib 的数值格式
    y = plot_df[y_col].to_numpy(dtype=float)#将 y_col 列转换为数值数组，非数值将被设置为 NaN
    rsi = plot_df[rsi_col].to_numpy(dtype=float)#将 rsi_col 列转换为数值数组，非数值将被设置为 NaN
    #创建线段，每个线段连接连续的两个点，-1自动计算行数，1增加一个维度（点），2表示每个点有两个坐标（x和y）
    points = np.column_stack([x, y]).reshape(-1, 1, 2)
    #取起始点和结束点，沿着行方向连接成线段，每个线段由两个点组成
    segments = np.concatenate([points[:-1], points[1:]], axis=1)

    if color_by_rsi:
        #根据 RSI 阈值为每个线段分配颜色。线段的颜色取决于其结束点的 RSI 值：
        colors = np.where(
            rsi[1:] > rsi_high,
            "red",
    ...
    #创建一个 LineCollection 对象，将线段和对应的颜色传递给它，并设置线宽和 zorder（绘制顺序）
    lc = LineCollection(
        segments,
        colors=colors,
...
#传入数据框和相关参数，绘制价格、成交量、RSI 图，并根据 RSI 阈值连续变色
def plot_analysis(
    df: pd.DataFrame,
    symbol: str,
    ...
    #show_daily_signals=False 时，不再按日线 RSI 给线段变色
    daily_color_by_rsi = bool(show_daily_signals)

    #根据 RSI 阈值连续变色绘制收盘价曲线
    ...
    #扩大一点 y 方向留白，给上方三角形/星星和下方三角形/星星留出空间
    axes[0].margins(y=0.20)
    axes[0].autoscale_view()

    #在收盘价曲线上额外标注周线 RSI 和月线 RSI 的极端区间信号
    if show_weekly_signals:
        _annotate_period_rsi_signals(
            ...
            high_marker="▲",       #周线 RSI > 阈值：红色上三角
            low_marker="▼",        #周线 RSI < 阈值：黑色下三角
            high_offset_points=10, #周线标记距离收盘价线段的垂直偏移，单位为屏幕坐标点（points）。正值表示在上方，负值表示在下方。
            low_offset_points=10,  #周线标记距离收盘价线段的垂直偏移，单位为屏幕坐标点（points）。正值表示在上方，负值表示在下方。
            marker_fontsize=13,    #周线标记的字体大小
        )
    ...
    if show_monthly_signals:
        _annotate_period_rsi_signals(
            ...
            high_marker="★",       #月线 RSI > 阈值：红色星星
            low_marker="★",        #月线 RSI < 阈值：黑色星星
            high_offset_points=15,  #月线标记距离收盘价线段的垂直偏移，单位为屏幕坐标点（points）。正值表示在上方，负值表示在下方。
            low_offset_points=15,   #月线标记距离收盘价线段的垂直偏移，单位为屏幕坐标点（points）。正值表示在上方，负值表示在下方。
            marker_fontsize=18,     #月线标记的字体大小
        )

    #绘制成交量柱状图；如果没有有效成交量，则显示提示文本
    _plot_volume_bar(axes[1], df)

    #根据 RSI 阈值连续变色绘制 RSI 曲线
    _plot_segmented_by_rsi(
        ax=axes[2],
        df=df,
        y_col=rsi_col,
    ...
    #在 RSI 图上添加水平线表示高低阈值，并设置 y 轴范围为 0-100
    ...
    if output_two_file:
        ...
        if rsi_col not in df.columns:
            raise KeyError(f"数据中找不到 RSI 列: {rsi_col}")
        #创建一个包含两个子图的图形，大小为 12x8 英寸，子图共享 x 轴
        ...
        plt.savefig(output_two_file, dpi=200)
        # print(f"图表已保存: {output_file}")
...
#统一外部调用入口：获取数据、计算 RSI、打印摘要、绘图，并返回完整 DataFrame
def rsi_analyze_index(
    ...
    修正版关键逻辑：
        1. 日线 RSI 不再只用展示区间计算，而是先在 raw_hist 长历史上计算，再截取最近 days。
        2. 日线、周线、月线 RSI 均使用同一个修正版 Wilder RSI。
    ...
    """
    #旧参数兼容：没有单独设置时，默认使用 rsi_window/rsi_high/rsi_low
    ...
    #兼容旧参数 show_period_markers
    if not show_period_markers:
        show_weekly_signals = False
        show_monthly_signals = False

    #为了计算月线 RSI，需要比最终展示区间更长的日线数据
    #例如 RSI(9) 的月线至少需要 9 个以上月度收盘价，实际建议多取一些历史数据
    ...
    )# 获取较长历史数据，用于计算日线/周线/月线 RSI

    #关键修正：日线 RSI 先在长历史 raw_hist 上计算，再截取最近 days，避免展示区间左侧 RSI 失真
    ...
    hist = raw_daily_df.tail(days).copy()# 最终展示最近 days 个交易日，RSI 已经由长历史计算得到
    ...
    if do_print:
        ...
        # print("\nRSI 参数设置：")
        # print(f"日线 RSI: window={daily_rsi_window}, high={daily_rsi_high}, low={daily_rsi_low}, show={show_daily_signals}")
        # print(f"周线 RSI: window={weekly_rsi_window}, high={weekly_rsi_high}, low={weekly_rsi_low}, show={show_weekly_signals}")
        # print(f"月线 RSI: window={monthly_rsi_window}, high={monthly_rsi_high}, low={monthly_rsi_low}, show={show_monthly_signals}")

        # print("\n周期 RSI 信号统计：")
        # print(f"日线 RSI 高/低信号数量: {0 if daily_signal_df is None else len(daily_signal_df)}")
        # print(f"周线 RSI 高/低信号数量: {0 if weekly_signal_df is None else len(weekly_signal_df)}")
        # print(f"月线 RSI 高/低信号数量: {0 if monthly_signal_df is None else len(monthly_signal_df)}")
