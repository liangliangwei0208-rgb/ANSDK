...
# 下面是一些辅助函数，用于计算价格变化、添加量化因子、格式化输出等。
...
# 主函数：生成分析结果文本和图片路径，供 main.py 调用。
def build_stock_analysis(
    ...
    """
    生成所有 RSI 图片，并返回邮件正文和图片路径。

    include_factors=True:
        正文加入趋势、动量、波动率、成交量比、回撤、ATR。

    include_realtime=True:
        对中国场内 ETF 合并盘中实时行情，便于午盘查看今天临时涨跌。
        盘中 RSI 和量化因子都是临时值，非收盘确认值。

    include_factors=False:
        正文只保留最新交易日、最新收盘、最新涨跌、成交量。
    """
    # 纳斯达克综合指数
    nasdaq_hist, nasdaq_daily_signal_df, nasdaq_weekly_signal_df, nasdaq_monthly_signal_df, nasdaq_signal_table, nasdaq_weekly_df, nasdaq_monthly_df = rsi_analyze_index(
        symbol=".IXIC",
        ...
        return_signals=True,
        show_plot=False, # 不显示图片
    )

    nasdaq_result = StockAnalysisResult(
        name="纳斯达克综合指数",
        hist=nasdaq_hist,
        weekly_df=nasdaq_weekly_df,
        monthly_df=nasdaq_monthly_df,
        daily_signal_df=nasdaq_daily_signal_df,
        weekly_signal_df=nasdaq_weekly_signal_df,
        monthly_signal_df=nasdaq_monthly_signal_df,
        signal_table=nasdaq_signal_table,
    )

    # 红利低波华泰ETF
    hongli_hist, hongli_daily_signal_df, hongli_weekly_signal_df, hongli_monthly_signal_df, hongli_signal_table, hongli_weekly_df, hongli_monthly_df = rsi_analyze_index(
        symbol="512890",
    ...
    hongli_result = StockAnalysisResult(
        name="红利低波华泰ETF",
        hist=hongli_hist,
        weekly_df=hongli_weekly_df,
        monthly_df=hongli_monthly_df,
        daily_signal_df=hongli_daily_signal_df,
        weekly_signal_df=hongli_weekly_signal_df,
        monthly_signal_df=hongli_monthly_signal_df,
        signal_table=hongli_signal_table,
    )

    # 上证指数 ETF：510210
    sz_hist, sz_daily_signal_df, sz_weekly_signal_df, sz_monthly_signal_df, sz_signal_table, sz_weekly_df, sz_monthly_df = rsi_analyze_index(
        symbol="510210",
    ...
    sz_result = StockAnalysisResult(
        name="上证指数ETF：510210",
        hist=sz_hist,
        weekly_df=sz_weekly_df,
        monthly_df=sz_monthly_df,
        daily_signal_df=sz_daily_signal_df,
        weekly_signal_df=sz_weekly_signal_df,
        monthly_signal_df=sz_monthly_signal_df,
        signal_table=sz_signal_table,
    )
    # 汇总结果
    results = [
        nasdaq_result,
